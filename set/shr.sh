...
a_xall() {
	...
	systemd_check "$SYSD_CHECK"
    	...
    	setup_smb  "$SMB_HEADER1" "$SHARED_FOLDER1" "$USERNAME1" "$SMB_PASSWORD1" "$WRITABLE_YESNO1" "$GUESTOK_YESNO1" "$BROWSABLE_YESNO1" 
    	setup_smb  "$SMB_HEADER2" "$SHARED_FOLDER2" "$USERNAME2" "$SMB_PASSWORD2" "$WRITABLE_YESNO2" "$GUESTOK_YESNO2" "$BROWSABLE_YESNO2" 
    	setup_smb_firewalld
}
