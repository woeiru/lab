...
`doc/pro/` to root `wow/` while preserving behavior, validation, and operator
muscle memory.

## Context

1. The previous workflow state and task templates lived under `doc/pro/`.
2. The requested target is a top-level `wow/` directory as the canonical home.
3. Existing references, scripts, and guardrails previously pointed to
   `doc/pro/`
   and must be updated consistently to avoid checker and workflow breakage.
...
1. Canonical board root changes from `doc/pro/` to `wow/`.
2. Canonical checker command changes from `bash doc/pro/check-workflow.sh` to
   `bash wow/check-workflow.sh`.
3. Canonical task template entrypoints change from `doc/pro/task/*` to
   `wow/task/*`.

### Mapping and invariants

1. Apply path-parity mapping for all board content:
   `doc/pro/<segment>` -> `wow/<segment>`.
...
1. Use hard cutover in tracked docs/scripts to remove `doc/pro/` as the
   primary
   path.
...
2. Move `doc/pro/` directory to root `wow/`.
3. Rewrite tracked references from `doc/pro/` to `wow/` where they are
...
1. Inventory all repository references to `doc/pro/` in scripts, docs, and task
   templates.
2. Define migration mapping from `doc/pro/**` to `wow/**` with path parity.
...
1. Partial path updates can leave hidden references to `doc/pro/` and cause
   workflow drift.
...
2. Phase 2 (Implementation) [done]: Apply one coordinated repository patch
   that moves `doc/pro/**` to `wow/**` and updates all in-repo
...
1. Pre-move baseline: run `bash doc/pro/check-workflow.sh` and record any
   existing failures.
2. Post-move workflow check: run `bash wow/check-workflow.sh` and require pass.
3. Reference audit: search for stale `doc/pro/` references in tracked docs and
...
1. Workflow board exists canonically at root `wow/` with the same operational
   structure previously used under `doc/pro/`.
...
1. Completed move: `doc/pro/` -> `wow/` with directory structure parity.
...
1. Moved workflow board root from `doc/pro/` to `wow/` with full folder
   structure parity.
2. Repointed active workflow docs, task templates, architecture/manual docs,
   and spec references from `doc/pro/` to `wow/`.
...
4. `git grep -n "doc/pro/" -- ':!doc/ref/stats/*' ':!STATS.md'` -> no active
   non-historical references remaining.
