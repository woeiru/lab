...
- Links: wow/task/RULES.md, wow/task/active-move, wow/README.md, wow/check-workflow.sh, wow/queue/20260303-2245_logging-performance-renewal-plan.md, wow/queue/20260303-2246_logging-architectural-restructure-plan.md, wow/queue/20260303-2247_logging-visual-output-redesign-plan.md
...
1. `wow/task/active-move` requires exact tokens: `Design: required` or
   `Design: not needed`.
2. Queue items commonly use `Design required: Yes/No` wording instead.
3. `wow/task/RULES.md` strict mode requires stop-on-missing rather than
   inference, so equivalent phrasing is treated as missing.
4. `wow/check-workflow.sh` does not validate triage design-classification
...
2. Update task templates and `wow/README.md` to match exactly.
...
2. Completed Phase 2 alignment by updating workflow guidance in
   `wow/task/active-capture` and `wow/README.md` to canonical
   `Design: required` / `Design: not needed` wording.
3. Completed Phase 3 checker enforcement in `wow/check-workflow.sh` for
...
4. Completed Phase 4 normalization for current queue items:
   - `wow/queue/20260303-2245_logging-performance-renewal-plan.md`
   - `wow/queue/20260303-2246_logging-architectural-restructure-plan.md`
   - `wow/queue/20260303-2247_logging-visual-output-redesign-plan.md`
5. Completed Phase 5 validation (`bash -n wow/check-workflow.sh`,
   `bash wow/check-workflow.sh`) and strict-mode precondition dry-run via
...
1. Run `wow/task/completed-close` for this item to move it to
   `wow/completed/` with final closure summary.
...
1. Update `wow/check-workflow.sh` to validate triage classification presence/format for queue and active items.
...
1. `bash -n wow/check-workflow.sh`
2. `bash wow/check-workflow.sh`
...
2. Updated workflow guidance to canonical wording in `wow/task/active-capture`
   and `wow/README.md`.
3. Implemented checker enforcement in `wow/check-workflow.sh` for queue/active
