...
### Function naming (global)
- **Public functions** MUST use a module prefix: `[module]_[name]` (example: `ana_laf`, `aux_val`).
- **Internal helpers** SHOULD use underscore-prefixed names scoped to module intent (example: `_ana_helper`).
- **Case** MUST be `snake_case` for function names and local variables.

### Documentation blocks (required by `ana`)
- `aux_use` relies on exactly three comment lines above each function.
- `aux_tec` relies on a structured technical block immediately inside each function.
- Minimum technical sections: **Technical Description**, **Dependencies**, **Arguments**.

### Validation and return codes (global)
- Help flags (`--help`/`-h`) MUST be handled first and return `0`.
- Parameters MUST be validated (prefer `aux_val` where available); invalid usage returns `1` and shows `aux_use`.
- Standard return codes: `0` success, `1` parameter/user error, `2` system/dependency/runtime failure, `127` missing dependency.

### Auxiliary integration boundaries
- `lib/gen` and `lib/ops` SHOULD use `aux_*` integration (`aux_val`, `aux_chk`, `aux_cmd`, `aux_ask`) where applicable.
- `lib/core` MUST remain bootstrap-safe and MUST NOT assume `aux_*` unless explicitly sourced.

### Quality and safety defaults
- Keep functions cohesive and typically under ~150 lines.
- Prefer `local` variables inside functions; use explicit module-level constants only when intentionally shared.
- Use safe file-operation patterns (backup/temporary files) for destructive edits.
- Provide clear, actionable errors before returning failure.
...
- For DIC-critical explicit execution behavior, follow `lib/ops/.spec`

**Stateless Design**: Functions should be environment-independent
- No hardcoded paths or values
- All configuration via parameters or environment variables (DIC mapping rules are canonical in `lib/ops/.spec`)
...
# Scope: All operational functions in lib/ops modules
# Companion: lib/ops/.spec (ops/DIC-specific technical requirements)
# Note: Legacy `.guide` content is merged into this file.
#
# Last Updated: 2025-06-14
# Maintainer: Lab Environment Management System
...
**DIC-critical execution contract:** The canonical rule for parameterless action functions and `-x` explicit execution lives in `lib/ops/.spec`.
