...
- Links: doc/pro/README.md, doc/pro/task/inbox-capture, doc/pro/task/active-split, doc/pro/check-workflow.sh
...
4. Extend `doc/pro/check-workflow.sh` with orchestration integrity checks
...
- [x] Add orchestration task templates under `doc/pro/task/` for fan-out,
  assignment, synchronization, and convergence.
- [x] Update `doc/pro/README.md` with parent/child orchestration semantics,
  dependency modeling, and wave-based execution guidance.
- [x] Update `doc/pro/task/README.md` and `doc/pro/task/RULES.md` so task
...
- [x] Extend `doc/pro/check-workflow.sh` to validate program/child references,
...
1. Updated orchestration guidance in `doc/pro/README.md` with parent/child
   model, required metadata, and wave operating cadence.
2. Added new orchestration task templates:
   `doc/pro/task/active-fanout`, `doc/pro/task/active-assign`,
   `doc/pro/task/active-sync`, `doc/pro/task/active-converge`.
3. Updated task-level docs and shared rules:
   `doc/pro/task/README.md`, `doc/pro/task/RULES.md`.
4. Extended `doc/pro/check-workflow.sh` with orchestration checks for:
...
1. Run `bash -n doc/pro/check-workflow.sh` after checker edits.
2. Run `bash doc/pro/check-workflow.sh` before each state transition.
...
1. Added large-initiative orchestration guidance to `doc/pro/README.md` with
   parent/child model, required metadata, and wave cadence.
2. Added orchestration task templates:
   `doc/pro/task/active-fanout`, `doc/pro/task/active-assign`,
   `doc/pro/task/active-sync`, and `doc/pro/task/active-converge`.
3. Updated `doc/pro/task/README.md` and `doc/pro/task/RULES.md` to document the
   orchestration mode and metadata contract.
4. Extended `doc/pro/check-workflow.sh` to enforce program sections, child
...
2. `bash doc/pro/check-workflow.sh` -> pass after orchestration updates and
...
1. Optional automation phase captured for future triage in
   `doc/pro/inbox/20260306-2351_orchestration-worktree-automation-phase-plan.md`.
