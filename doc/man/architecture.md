...
-   `lib/`: Shell script libraries and reusable modules.
    ...
    -   `ops/`: Source files for deployment modules (`gpu`, `net`, `pbs`, `pve`, `srv`, `sto`, `sys`, `usr`).
...
-   **Source Modules**: Available in `lib/ops/` with specialized modules for different system areas:
