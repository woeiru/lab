...
*   **Password Generation:** Use `sec_gen` to create secure passwords (e.g., from `/dev/urandom`).
...
Include context as key-value pairs when logging complex operations:
```bash
aux_info "component=gpu,operation=passthrough,status=started"
```
...
*   You can toggle this logging on or off using the `lo1_setlog on|off` command.
