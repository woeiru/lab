...
The Dependency Injection Container (DIC) is an intelligent parameter resolution engine bridging the gap between environment-specific configurations (`cfg/env/`) and the stateless pure functions in `lib/ops/`.
...
Functions in `lib/ops/` are pure—they require all parameters to be explicitly provided (e.g., `pve_vm_create "100" "ubuntu" "4096"`). 
...
```mermaid
graph TD
    A["CLI / Deployment Script (src/set/h1)"] -->|Call: ops pve vm_create -j| B["src/dic/ops (DIC Engine)"]
    ...
    D -->|Resolved Args| I["lib/ops/pve_vm_create"]
...
1.  **User Arguments:** Arguments passed directly via the CLI (e.g., `ops pve vm_create 100`).
...
*   **Explicit (`-x`):** Passes specific execution or validation flags directly to the underlying pure function, bypassing standard injection logic for specialized administrative tasks.