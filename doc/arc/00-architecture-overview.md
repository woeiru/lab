...
This repository is a Bash-native infrastructure automation system with two runtime families: (1) interactive shell bootstrap via `go` -> `bin/ini` -> `bin/orc`, and (2) execution flows that now center on declarative reconciliation (`cfg/dcl` -> `src/rec` -> `src/run`) while retaining legacy compatibility surfaces (`src/set`, `src/dic/ops`). This overview maps those boundaries and call flows; module-level internals are covered in `01` through `09`.
...
| Runtime constants/config | `cfg/core/ric`, `cfg/core/rdc`, `cfg/env/*`, `cfg/dcl/*` | Defines global paths/runtime variables, environment context overlays (`cfg/env`), and declarative desired state (`cfg/dcl`). |
| Reconciliation compiler | `src/rec/ops` | Validates declarative model and compiles deterministic plan artifacts (`rec-plan-v0`). |
| Run dispatcher | `src/run/dispatch`, `src/run/gate-evidence` | Validates plan/enforcement contracts and dispatches execution to legacy runbooks during migration. |
...
| Legacy deployment manifests | `src/set/*`, `src/set/.menu` | Host entrypoints retained for compatibility; by default delegate to `src/run/dispatch`, then execute section functions through DIC (`ops ...`). |
...
5. A deployment script (for example `src/set/h1`) first delegates to `src/run/dispatch` when available (default). If `LAB_USE_DIC_RUN_BRIDGE=1`, it first delegates to `src/dic/run`, which compiles a reconciliation artifact (`src/rec/ops compile`) and forwards into dispatch.
6. `src/run/dispatch` applies optional plan validation (`--plan`), enforcement-stage resolution (`compat|guarded|strict`), dependency/order/policy-gate checks, then re-execs the legacy `src/set/<target>` with `LAB_RUN_DISPATCH_BYPASS=1`.
7. In bypass mode, the target runbook sources `src/set/.menu` + `src/dic/ops`, routes `-i`/`-x` via `setup_main`, and executes selected section functions (`*_xall`).
8. Selected `*_xall` sections invoke `ops module function ...`; DIC resolves/injects arguments (`ops_main` -> `ops_execute` -> `ops_inject_and_execute`) and calls target `module_function` symbols from sourced files in `lib/ops/*`.
...
sequenceDiagram
    ...
    alt LAB_USE_DIC_RUN_BRIDGE=1
        S->>C: src/rec/ops compile (via src/dic/run)
        C-->>S: rec-plan artifact
        S->>R: dispatch h1 --plan <artifact> -x a
    else default
        S->>R: exec dispatch h1 -x a
    end
    R->>S: exec src/set/h1 with LAB_RUN_DISPATCH_BYPASS=1
    S->>M: source .menu
    S->>D: source ../dic/ops
    S->>S: setup_main("-x", "a")
    S->>D: ops pve dsr -j
...
flowchart LR
    ...
    G --> H[src/run/dispatch]
    H --> I{plan-aware mode?}
    I -->|yes| J[src/rec compile + contract checks]
    I -->|no| K[compat dispatch]
    J --> L[src/set/.menu routing]
    K --> L
    L --> M[src/dic/ops injection]
    M --> N[lib/ops module function]
    N --> O[Infrastructure operation]
```
...
- `src/set/*` entrypoints default to `exec` through `src/run/dispatch` when available; `LAB_RUN_DISPATCH_BYPASS=1` is used to prevent recursive re-dispatch.
...
- `src/set/*` falls back to direct legacy behavior when `src/run/dispatch` is unavailable/not executable.
- `src/run/dispatch` rejects enforcement flags and gate-evidence inputs when `--plan` is missing, and rejects invalid/ambiguous plan contracts before any runbook execution.
...
- Declarative intent is now expected under `cfg/dcl/*`; desired-state behavior changes should not originate from `cfg/env/*`.
- Deployment scripts in `src/set/*` remain tightly coupled to DIC command semantics (`ops module function -j`) after dispatch handoff, not direct function calls.
...
- `doc/arc/05-deployment-and-config.md`: `cfg/{env,dcl}` hierarchy and `src/{set,rec,run}` execution context.
...
- `doc/arc/08-workflow-architecture.md`: agent workflow coordination system (`wow/`), state machine, gate design, session handoff.
- `doc/arc/09-planning-subsystem.md`: local planning subsystem (`utl/pla`) command flow, data model boundaries, and artifact lifecycle.

## Maintenance Note

Update this file in the same PR when any of these change: primary entrypoints (`go`, `bin/ini`, `bin/orc`, `src/run/dispatch`, `src/rec/ops`, `src/set/.menu`, `src/dic/ops`), cross-layer load order, top-level directory responsibilities, or shell/runtime side-effect contracts.
